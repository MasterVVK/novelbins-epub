...
{"[...текст продолжается...]" if len(text) > 2000 else ""}"""
    ...
    def improve_text_style(self, text: str) -> str:
        ...
        if len(text) > 3000:
            parts = self.split_text_smart(text)
            improved_parts = []

            for i, part in enumerate(parts, 1):
                print(f"    Обработка части {i}/{len(parts)}...")

                prompt = f"""Улучши стилистику этой части текста:

{part}

Это часть {i} из {len(parts)} большого текста."""

                result = self.make_request(STYLE_IMPROVEMENT_PROMPT, prompt, temperature=0.7)
                if result:
                    improved_parts.append(result)
                else:
    ...
    def validate_edit(self, original: str, edited: str) -> bool:
        ...
        if ratio < 0.7 or ratio > 1.5:
            print(f"  ⚠️  Значительное изменение размера: {ratio:.2f}")
            return False
    ...
    def save_edited_chapter(self, chapter: Chapter, edited_text: str,
        ...
        cursor.execute("PRAGMA table_info(edited_chapters)")
        columns = [col[1] for col in cursor.fetchall()]

        if 'status' not in columns:
            cursor.execute("ALTER TABLE edited_chapters ADD COLUMN status TEXT DEFAULT 'completed'")

        if 'edit_count' not in columns:
            cursor.execute("ALTER TABLE edited_chapters ADD COLUMN edit_count INTEGER DEFAULT 1")
