...
Улучши стилистику этого текста, сохраняя его полноту и точность.

КРИТИЧЕСКИ ВАЖНО:
1. СОХРАНИ ВЕСЬ ТЕКСТ - не сокращай, не удаляй абзацы
2. СОХРАНИ ВСЕ ДЕТАЛИ - каждый факт, число, имя должны остаться
3. ТОЛЬКО УЛУЧШАЙ ФОРМУЛИРОВКИ - не меняй смысл
4. НЕ ДОБАВЛЯЙ вводные фразы типа "Вот исправленный текст"
...
ФОРМАТ ОТВЕТА:
Начни сразу с улучшенного текста без всяких вводных слов."""
...
КРИТИЧЕСКИ ВАЖНО:
- СОХРАНИ ВЕСЬ ТЕКСТ, не только диалоги
- НЕ СОКРАЩАЙ и НЕ УДАЛЯЙ ничего
- Работай ТОЛЬКО с репликами персонажей
...
- Верни ВЕСЬ текст, включая описания между диалогами"""
...
КРИТИЧЕСКИ ВАЖНО:
- СОХРАНИ ВЕСЬ ТЕКСТ в полном объёме
- НЕ СОКРАЩАЙ, только исправляй ошибки
...
ФОРМАТ: Верни ПОЛНЫЙ исправленный текст."""
...
{"[...текст продолжается...]" if len(text) > 2000 else ""}"""
    ...
    def clean_llm_response(self, text: str) -> str:
        """Очистка ответа от служебных фраз LLM"""
        # Паттерны вводных фраз, которые нужно удалить
        intro_patterns = [
            r'^Вот\s+(?:исправленный|отредактированный|улучшенный|отполированный).*?:\s*\n+',
            r'^Вот\s+(?:исправленный|отредактированный|улучшенный|отполированный).*?:\s*',
            r'^Исправленный текст:\s*\n*',
            r'^Отредактированный текст:\s*\n*',
            r'^Улучшенный текст:\s*\n*',
            r'^Отполированный текст:\s*\n*',
            r'^Вот текст.*?:\s*\n*',
            r'^Текст после редактуры:\s*\n*',
        ]
    ...
    def improve_text_style(self, text: str) -> str:
        ...
        if result:
            result = self.clean_llm_response(result)
