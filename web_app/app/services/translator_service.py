...
class TranslatorService:
    ...
    def extract_title_and_content(self, translated_text: str) -> Tuple[str, str]:
        ...
        Определяет, является ли первая строка заголовком главы.
        Основной признак: короткая первая строка, после которой идёт пустая строка.
        Дополнительно проверяются ключевые слова и номера глав.
        """
        ...
        # После первой строки идёт пустая строка — главный признак заголовка
        has_empty_line_after = len(lines) > 1 and lines[1].strip() == ""

        # Содержит ключевые слова заголовка
        ...
        # Заканчивается знаками препинания обычного текста = вероятно НЕ заголовок
        ...
        # Решение: первая строка — заголовок, если:
        # 1. Короткая + после неё пустая строка (основной признак)
        # 2. Или: содержит ключевое слово/номер (старая логика)
        is_likely_title = (
            (is_short and has_empty_line_after and not ends_with_punctuation) or
    ...
    def _find_glossary_conflicts(self, new_terms: Dict, novel_id: int) -> list:
        ...
        for category, terms in new_terms.items():
            for eng, rus in terms.items():
                ...
                # Ищем базовые термины, которые являются ПРЕФИКСОМ нового
                # Префикс, а не просто подстрока — чтобы избежать ложных срабатываний:
                # 赤母 → 赤母凡蛻 (префикс, реальный конфликт) ✓
                # 道剑 → 诡道剑 (суффикс, разные слова: 诡道+剑) ✗
                for base_zh, base_ru in existing_map.items():
                    if len(base_zh) >= len(eng_norm):
                        continue  # Базовый не короче нового
                    if not eng_norm.startswith(base_zh):
                        continue  # Базовый не является префиксом нового
